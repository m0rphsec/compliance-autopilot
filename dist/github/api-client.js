...
exports.GitHubClient = exports.PermissionError = exports.RateLimitError = exports.GitHubAPIError = void 0;
...
class RateLimitError extends GitHubAPIError {
    ...
    limit;
    remaining;
    constructor(message, resetAt, limit, remaining) {
...
class PermissionError extends GitHubAPIError {
    requiredScopes;
...
class GitHubClient {
     ...
     */
    async getDeployments(limit = 100) {
        try {
     ...
     */
    async getIssues(labels, state = 'all') {
        try {
    ...
    async getVulnerabilityAlerts() {
        try {
            ...
            const response = await this.octokit.request('GET /repos/{owner}/{repo}/vulnerability-alerts', {
                owner: this.owner,
                repo: this.repo,
            });
            return response.data || [];
     ...
     */
    handleError(error, context, requiredScopes) {
        // Rate limit error
        if (this.isRateLimitError(error)) {
            ...
            return new RateLimitError(`${context}\n\n` +
                ...
                `The action will automatically retry with exponential backoff.`, resetDate, limit, remaining);
        }
        // Permission error
        if (this.isPermissionError(error)) {
            const scopesMessage = requiredScopes
                ...
                : '';
            return new PermissionError(`${context}\n\n` +
                ...
                `     issues: read\n` +
                `     deployments: read\n\n` +
                `Or use a Personal Access Token with full repo access.`, requiredScopes || []);
        }
        // Not found error
        if (typeof error === 'object' &&
            ...
            return new GitHubAPIError(`${context}\n\n` +
                ...
                `Please check the repository name and resource identifier.`, 404, 'NOT_FOUND');
        }
        ...
        if (typeof error === 'object' &&
            ...
            return new GitHubAPIError(`${context}\n\n` +
                ...
                `This is usually temporary. The action will automatically retry.`, undefined, 'NETWORK_ERROR');
        }
        // Generic error
        ...
        const errorStatus = typeof error === 'object' && error !== null && 'status' in error
            ? error.status
            : undefined;
        return new GitHubAPIError(`${context}\n\n` + `Unexpected error: ${errorMessage}\n` + `Status: ${errorStatus || 'N/A'}`, errorStatus, 'UNKNOWN');
    }
}
