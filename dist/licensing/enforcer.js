...
class LicenseEnforcer {
    ...
    enforce(context) {
        ...
        const blockedFrameworksList = adjustedFrameworks.filter((fw) => !this.limits.frameworks.includes(fw));
        if (blockedFrameworksList.length > 0) {
            blockedFeatures.push(...blockedFrameworksList.map((fw) => `framework-${fw}`));
            warnings.push(`Framework(s) ${blockedFrameworksList.join(', ')} require a paid plan. ` +
                `Using available frameworks: ${allowedFrameworks.join(', ')}`);
            adjustedFrameworks = allowedFrameworks;
        }
        // Check PDF reports
        if ((context.reportFormat === 'pdf' || context.reportFormat === 'both') &&
        ...
        const allowed = adjustedFrameworks.length > 0 && (this.limits.privateRepos || !context.isPrivateRepo);
        return {
            allowed,
     ...
     */
    static getUpgradePrompt(blockedFeatures) {
        const lines = [
            '',
            '╔══════════════════════════════════════════════════════════════╗',
