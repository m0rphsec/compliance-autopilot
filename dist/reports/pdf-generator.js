...
class PDFGenerator {
    ...
    async generateCoverPage(data) {
        ...
        if (data.frameworkSummaries && data.frameworkSummaries.length > 0) {
            this.currentY -= 10;
            const fwParts = data.frameworkSummaries.map((fw) => `${fw.framework}: ${fw.passed}/${fw.total} passed`);
    ...
    async generateControlFindings(data) {
        ...
        const sortedKeys = Object.keys(grouped).sort((a, b) => frameworkOrder.indexOf(a) - frameworkOrder.indexOf(b));
        for (const fw of sortedKeys) {
            ...
            const statusOrder = { PASS: 0, PARTIAL: 1, FAIL: 2, NOT_APPLICABLE: 3 };
            const sorted = [...controls].sort((a, b) => (statusOrder[a.status] ?? 4) - (statusOrder[b.status] ?? 4));
    ...
    groupControlsByFramework(controls) {
        const grouped = {};
    ...
    drawFrameworkHeader(framework, _controls, summaries) {
    ...
    async generateRecommendations(data) {
        ...
        for (const fw of sortedKeys) {
            ...
            const sorted = [...fwControls].sort((a, b) => (severityOrder[a.severity] ?? 4) - (severityOrder[b.severity] ?? 4));
            for (const control of sorted) {
