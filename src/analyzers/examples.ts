...
import { AnalysisRequest, BatchAnalysisRequest } from '../types/analyzer';
...
async function example5_errorHandling() {
  ...
  const limitedAnalyzer = new CodeAnalyzer(process.env.ANTHROPIC_API_KEY!, {
...
async function example7_performanceBenchmark() {
  ...
  const files: AnalysisRequest[] = Array.from({ length: 100 }, (_, i) => ({
    code: `
// File ${i}
const value = ${i};
...
export {
  example1_detectHardcodedSecrets,
  example2_detectPII,
  example3_batchAnalysis,
  example4_cacheDemo,
  example5_errorHandling,
  example6_multiFramework,
