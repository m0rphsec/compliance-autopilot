...
export class GDPRCollector {
  private anthropic: Anthropic;
  private piiDetector: PIIDetector;
  private cache: Map<string, PIIDetectionResult>;
   ...
   */
  async scanFile(code: string, _filePath: string): Promise<PIIDetectionResult> {
    // Check cache first
    ...
    let claudeAnalysis: Partial<PIIDetectionResult> = {};
   ...
   */
  async scanRepository(files: ScanFileInput[]): Promise<GDPRCollectorResult> {
    ...
    // Aggregate results
    const allViolations: GDPRViolation[] = [];
    const allPIIMatches: PIIMatch[] = [];
    let filesWithPII = 0;
  ...
  private async analyzeWithClaude(
    code: string,
    _filePath: string
  ): Promise<Partial<PIIDetectionResult>> {
...
Return JSON:
{
  ...
  recommendations: string[]
    ...
    const response = await this.anthropic.messages.create({
      ...
      max_tokens: 1024,
      system:
        'You are a GDPR compliance expert. Analyze code and return structured JSON responses.',
      messages: [
        {
          role: 'user',
          content: prompt,
   ...
   */
  private detectEncryptionRest(code: string): boolean {
    const patterns = [
      /encrypt\(/i,
   ...
   */
  private detectConsentMechanism(code: string): boolean {
    const patterns = [
      /consent/i,
      /gdpr.*accept/i,
      /checkbox.*agree/i,
   ...
   */
  private detectRetentionPolicy(code: string): boolean {
    const patterns = [
      /TTL|expir/i,
      /retention/i,
      /deleteAfter/i,
   ...
   */
  private detectDeletionCapability(code: string): boolean {
    const patterns = [
      /\.delete\s*\(/i,
      /\.destroy\s*\(/i,
      /\.remove\s*\(/i,
   ...
   */
  private inferCollectionMethods(code: string): string[] {
   ...
   */
  private calculateCompliance(result: PIIDetectionResult): boolean {
    if (!result.has_pii) {
   ...
   */
  private getRecommendation(violation: string): string {
    const recommendations: Record<string, string> = {
      ...
      'deletion capability':
        'Create DELETE endpoints for user data removal. Implement cascading deletes.',
   ...
   */
  private calculateRepositoryScore(fileResults: Array<{ result: PIIDetectionResult }>): number {
    ...
    fileResults.forEach(({ result }) => {
      if (result.has_pii) {
        ...
        let fileScore = 0;
        if (result.encryption_transit) fileScore += 20;
        if (result.encryption_rest) fileScore += 20;
        if (result.consent_mechanism) fileScore += 20;
        if (result.retention_policy) fileScore += 20;
        if (result.deletion_capability) fileScore += 20;
