...
const logger = createLogger('iso27001-collector');
...
interface ISO27001ControlDef {
  name: string;
  description: string;
  category: string;
}

const ISO27001_CONTROL_DEFS: Record<string, ISO27001ControlDef> = {
  'A.9.2.3': {
...
export class ISO27001Collector {
  ...
  private config: ISO27001CollectorConfig;
   ...
   */
  private async evaluateA9_2_3(): Promise<ControlEvaluation> {
    ...
    const adminCount = collaborators.filter(
      (c) => c.permissions?.admin || c.role_name === 'admin'
    ).length;
    ...
    return {
      ...
      framework: ComplianceFramework.ISO27001,
      result: pass ? ControlResult.PASS : ControlResult.PARTIAL,
      evidence,
   ...
   */
  private async evaluateA9_4_1(): Promise<ControlEvaluation> {
    ...
    if (branchProtection) {
      evidence.push(
        this.createEvidence('api_response', 'github_branch_protection', {
    ...
    return {
      ...
      controlName: control.name,
      framework: ComplianceFramework.ISO27001,
      result: hasRestrictions ? ControlResult.PASS : ControlResult.FAIL,
      evidence,
   ...
   */
  private async evaluateA12_1_2(): Promise<ControlEvaluation> {
   ...
   */
  private async evaluateA12_6_1(): Promise<ControlEvaluation> {
    ...
    try {
      ...
      hasSecurityWorkflow = workflows.workflows.some(
        (w) =>
   ...
   */
  private async evaluateA14_2_2(): Promise<ControlEvaluation> {
   ...
   */
  private async evaluateA14_2_5(): Promise<ControlEvaluation> {
    ...
    return {
      ...
      controlName: control.name,
      framework: ComplianceFramework.ISO27001,
      result: hasSecurityPolicy ? ControlResult.PASS : ControlResult.PARTIAL,
      evidence,
   ...
   */
  private async evaluateA14_2_8(): Promise<ControlEvaluation> {
    ...
    const testWorkflows = workflows.workflows.filter(
      (w) =>
   ...
   */
  private async evaluateA16_1_2(): Promise<ControlEvaluation> {
   ...
   */
  private async evaluateA16_1_5(): Promise<ControlEvaluation> {
   ...
   */
  private createErrorResult(controlId: string, error: unknown): ControlEvaluation {
   ...
   */
  private calculateSummary(evaluations: ControlEvaluation[]) {
    const passed = evaluations.filter((e) => e.result === ControlResult.PASS).length;
    const failed = evaluations.filter((e) => e.result === ControlResult.FAIL).length;
    const partial = evaluations.filter((e) => e.result === ControlResult.PARTIAL).length;
    const error = evaluations.filter((e) => e.result === ControlResult.ERROR).length;
    const notApplicable = evaluations.filter(
