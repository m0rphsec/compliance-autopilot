...
interface SOC2ControlDef {
  name: string;
  description: string;
}

const SOC2_CONTROL_DEFS: Record<string, SOC2ControlDef> = {
  'CC1.1': {
...
export class SOC2Collector {
  ...
  private config: SOC2CollectorConfig;
   ...
   */
  private async evaluateCC1_1(): Promise<ControlEvaluation> {
    ...
    const requiresReview =
      (branchProtection?.required_pull_request_reviews?.required_approving_review_count ?? 0) > 0;

    return {
      controlId: 'CC1.1',
      controlName: control.name,
      framework: ComplianceFramework.SOC2,
      result: requiresReview ? ControlResult.PASS : ControlResult.FAIL,
      evidence,
   ...
   */
  private async evaluateCC6_1(): Promise<ControlEvaluation> {
    ...
    let deployments: { id: number; environment: string; created_at: string }[] = [];
    try {
    ...
    return {
      ...
      controlName: control.name,
      framework: ComplianceFramework.SOC2,
      result: hasWorkflows ? ControlResult.PASS : ControlResult.FAIL,
      evidence,
      notes: hasWorkflows
   ...
   */
  private async evaluateCC6_6(): Promise<ControlEvaluation> {
   ...
   */
  private async evaluateCC7_1(): Promise<ControlEvaluation> {
